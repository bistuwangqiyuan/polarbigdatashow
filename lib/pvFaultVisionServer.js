import { PV_VISION_JSON_INSTRUCTION, extractJsonObject, normalizePvFaultReport } from 'lib/pvFaultVisionSchema'

/**
 * 通用 OpenAI 兼容接口调用（Groq / Mistral / 智谱 / DashScope 等均复用）
 * @param {{ baseUrl: string, apiKey: string, models: string[], providerTag: string, mimeType: string, base64NoPrefix: string, supportsJsonMode?: boolean }} opts
 */
async function callOpenAICompatVision({
  ...
  const prompt = `${PV_VISION_JSON_INSTRUCTION}\n\n只输出 JSON 对象本身，不要 markdown 代码块。`
  ...
  for (const model of models) {
    ...
    try {
      ...
      return { ok: true, report: normalizePvFaultReport(parsed, `${providerTag}:${model}`) }
    } catch (e) {
...
export function analyzeWithGroq(base64NoPrefix, mimeType, apiKey) {
...
export function analyzeWithMistral(base64NoPrefix, mimeType, apiKey) {
  return callOpenAICompatVision({
...
export function analyzeWithZhipuGLM(base64NoPrefix, mimeType, apiKey) {
  return callOpenAICompatVision({
...
export function analyzeWithDashScopeQwenVL(base64NoPrefix, mimeType, apiKey) {
  const baseUrl =
...
export async function analyzeWithGemini(base64NoPrefix, mimeType, apiKey) {
  const prompt = `${PV_VISION_JSON_INSTRUCTION}\n\n只输出 JSON，键名必须与说明一致。`
  let lastErr = 'unknown'
  for (const model of GEMINI_MODELS) {
    ...
    try {
      ...
      return { ok: true, report: normalizePvFaultReport(parsed, `gemini:${model}`) }
    } catch (e) {
...
export async function analyzeWithOpenAI(base64NoPrefix, mimeType, apiKey) {
  const prompt = `${PV_VISION_JSON_INSTRUCTION}\n\n只输出 JSON 对象本身。`
  const dataUrl = `data:${mimeType || 'image/png'};base64,${base64NoPrefix}`
  try {
    ...
    return { ok: true, report: normalizePvFaultReport(parsed, 'openai:gpt-4o') }
  } catch (e) {
